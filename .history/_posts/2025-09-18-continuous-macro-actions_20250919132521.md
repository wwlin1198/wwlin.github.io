...
When the option terminates (at primitive step \(\tau_i\)), there will be a new \emph{macro-observation} \(z^i\) via
...
\(\text{(How good is } h \text{ over policy)}\)

\[ Q^{\Psi}(h, m) = r^c(h, m) + \int_{h'} P(h'\mid h, m)\, V^{\Psi}(h')\, dh' \]\
\(\text{(How good is taking macro-action over history)}\)
